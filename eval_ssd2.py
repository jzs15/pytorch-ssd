...
if __name__ == '__main__':
    ...
    if args.net == 'vgg16-ssd':
        ...
        predictor = create_mobilenetv2_ssd_lite_predictor(net, nms_method=args.nms_method, device=DEVICE, config=config)
    else:
        logging.fatal("The net type is wrong. It should be one of vgg16-ssd, mb1-ssd and mb1-ssd-lite.")
        parser.print_help(sys.stderr)
        sys.exit(1)
    ...
    for bound in iou:
        ...
        #        print("Prediction: {:4f} seconds.".format(timer.end("Predict")))
            ...
            aps = []
            print(f"\nAverage Precision Per-class {bound}:")
            for class_index, class_name in enumerate(class_names):
                if class_index == 0:
                    continue
                prediction_path = eval_path / f"det_test_{class_name}.txt"
            ...
            print(f"Average Precision Across All Classes:{sum(aps)/len(aps)}")

            print(cal_boxdiff(predictor, dataset, bound))
            print(tfpercent(predictor, dataset, bound))
